...
```{r initial data analysis from scaled dataset}
...
```{r clustered mean trajectories Figure 2+3}
...
```{r Figure 4 Effect sizes on BPD/Sepsis}
...
#do mice imputation beforehand
freq_list<-apply(freq[1:43], 2, FUN = function(x) { x<-data.frame(x, "timepoint"=metadata$timepoint_cat, "chip"=metadata$chip_id, "patient"=metadata$MHH_PatID )%>%          
                                                    pivot_wider(id_cols = patient, names_from=timepoint, values_from = c(1), id_expand=TRUE, names_sort = TRUE)
                                                    colnames(x)[2:4]<-c("t1", "t2", "t3")
                                                    return(x)})
...
clusterExport(cl=cl, varlist = c("mice_impute_rf", "freq_list", "%>%"))
freq_imputed<-parLapply(cl=cl, X=freq_list, fun=function(x)tryCatch({mice_impute_rf(x, donorID="patient")}, error=function(e){return(e)}) )
#freq_imputed<-lapply(freq_list, FUN=function(x) mice_impute_rf(x, donorID="patient"))
stopCluster(cl)

#remoce imputations where calculation failed
freq_imputed[lengths(freq_imputed)==4]->freq_imputed

data.table::rbindlist(freq_imputed[1:length(freq_imputed)], idcol="celltype")->freq_imputed
rbind(pivot_wider(freq_imputed, id_cols = patient, names_from = celltype, values_from = "1")%>%mutate("timepoint"=rep(1, 114)),
      pivot_wider(freq_imputed, id_cols = patient, names_from = celltype, values_from = "2")%>%mutate("timepoint"=rep(2, 114)),
      pivot_wider(freq_imputed, id_cols = patient, names_from = celltype, values_from = "3")%>%mutate("timepoint"=rep(3, 114)))->freq_imputed

#calculate gap statistic for imputed data and original data
gap_statisic<-function(freq, output_name, ncol){
gap_results<-list()
library(progress)
pb<-progress_bar$new(
  format = "  Gap statisic [:bar] :percent eta: :eta",
  total = length(ncol), clear = FALSE, width= 60)
freq<-freq[ncol]
ncol<-1:(ncol(freq))
for (i in ncol){
  if(nrow(freq)==252){
  data.frame(freq[,i], "timepoint"=metadata$timepoint_cat, "chip"=metadata$chip_id, "patient"=metadata$MHH_PatID )%>%          
  pivot_wider(id_cols = patient, names_from=timepoint, values_from = colnames(freq)[i], id_expand=TRUE, names_sort = TRUE)->data}
  else if (nrow(freq)==342){
  data.frame(freq[,i], "timepoint"=freq_imputed$timepoint, "patient"=freq_imputed$patient)%>%          
  pivot_wider(id_cols = patient, names_from=timepoint, values_from = colnames(freq)[i], id_expand=TRUE, names_sort = TRUE)->data
  }
  
  gap_results[[i]]<-clusGap(na.omit(data[2:4]), motifcluster::kmeanspp, K.max=7, B=500, verbose = FALSE)
  pb$tick()
}

names(gap_results)<-colnames(freq)[ncol]

best_k<-function(x){maxSE(f=x[["Tab"]][,3], x[["Tab"]][,4])}

lapply(gap_results, best_k)->best_k

plot_clusgap <- function(clusgap, title="Gap Statistic results"){
	gstab = data.frame(clusgap$Tab, k = 1:nrow(clusgap$Tab))
	p = ggplot(gstab, aes(k, gap)) + geom_line() + geom_point(size = 1)
	p = p + geom_errorbar(aes(ymax = gap + SE.sim, ymin = gap - SE.sim))
	p = p + ggtitle(title)+theme(plot.title = element_text(size=7))
	return(p)
}

lapply(seq(names(gap_results)), function(x) plot_clusgap(gap_results[[x]], title=names(gap_results)[x]))->gap_results

for(i in ncol){
  if(best_k[[i]]>1){
    gap_results[[i]]<-gap_results[[i]]+
                      geom_vline(xintercept=best_k[[i]], size=1)+
                      theme(panel.border = element_rect( color="green", fill=NA, size= 1))
  } else {
    gap_results[[i]]<-gap_results[[i]]+geom_vline(xintercept=best_k[[i]], size=1)}
}

cowplot::plot_grid(plotlist=gap_results)->p

pdf(output_name, width = 14, height = 10)
print(p)
dev.off()
...
gap_statisic(freq, "clusGap_result_original.pdf", ncol=1:44)->clusGab_results_original
gap_statisic(freq_imputed, "clusGap_result_imputed.pdf", ncol=2:(ncol(freq_imputed)-1))->clusGap_results_imputed

saveRDS(freq_imputed, "freq_imputed.rds")
```

#kml clustering on imputed data
```{r}

#do it in a for loop

trajectories<-list()
clusters<-data.frame(rep("patient", 114))
colnames(clusters)<-"patient"

for(i in 2:(ncol(freq_imputed)-1)){
  
  set.seed(12345)
  
  data.frame(freq_imputed[,i], "timepoint"=freq_imputed$timepoint, 
             "patient"=freq_imputed$patient)%>%          
  pivot_wider(id_cols = patient, names_from=timepoint, values_from = colnames(freq_imputed)[i], id_expand=TRUE, names_sort = TRUE)%>%
  na.omit()->subdata

              
  clusters$patient<-subdata$patient
  
  clusterLongData(subdata[2:4], idAll = subdata$patient, maxNA = 2)->cld

  cld["initializationMethod"] <- "kmeans++"
  
  kml(cld, nbRedrawing = 1000, nbClusters = 2:6, parAlgo = parALGO(startingCond = "kmeans++", imputationMethod = "copyMean", scale = T, saveFreq = Inf))
  ...
  trajectories[[colnames(freq_imputed)[i]]][["cld"]]<-cld
  clusters<-cbind(clusters, kml::getClusters(xCld=cld, nbCluster = which(crit==max(crit))+1))
  colnames(clusters)[ncol(clusters)]<-colnames(freq_imputed)[i]
